...
**Текущее состояние (v3):** Компрессия контекста, RAG (ChromaDB), bounded memory, safe_edit, ask_human, set_goal/read_goals, add_tool, browse_web, rag_index_evolution, rag_list, rag_fetch, runner как источник истины, capability-gate. 32 инструмента. **Инфраструктура самоулучшения:** evaluator, self_improve (run_one_cycle), evolution_log.jsonl, protected paths. **Blueprint Фаза 1 (выполнена):** Capability zones, Metacognitive monitor, Forgetting curves. **Фаза 2.1 (выполнена):** Memory Hierarchy (Letta-style): Core → Recall → Archival. **Фаза 2.2 (выполнена):** Sleep-Time Consolidation: episodic → semantic, пишет в RAG. **Следующий этап — Фаза 2.3:** Curiosity-Driven Discovery. См. [README.md](README.md).
